...
class BufferWriter {
public:
    BufferWriter() : begin(), current(), end() {}

    /// @brief Constructor
    /// @param begin Begin of data to write to
    /// @param end End of data to write to
    BufferWriter(uint8_t *buffer, uint8_t *end) : begin(buffer), current(buffer), end(end) {}
    BufferWriter(char *buffer, char *end) : begin((uint8_t *)buffer), current((uint8_t *)buffer), end((uint8_t *)end) {}

    /// @brief Constructor
    /// @param data Data to write to
    /// @param length Length of data to write to
    BufferWriter(uint8_t *buffer, int length) : begin(buffer), current(buffer), end(buffer + length) {}
    BufferWriter(char *buffer, int length) : begin((uint8_t *)buffer), current((uint8_t *)buffer), end((uint8_t *)(buffer + length)) {}

    /// @brief Constructor for buffer supporting std::data() and std::size()
    /// @tparam T Buffer type
    /// @param buffer Buffer to write to
    template <typename T>
    BufferWriter(T &buffer) : begin(std::data(buffer)), current(this->begin), end(this->begin + std::size(buffer)) {}
    template <typename T>
    BufferWriter(T &&buffer) : begin(std::data(buffer)), current(this->begin), end(this->begin + std::size(buffer)) {}
